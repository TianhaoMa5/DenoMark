...
`DenoMark-Encoder` is the semantic encoder checkpoint used for the paper's
reported DenoMark results. Download it from:

```text
https://github.com/TianhaoMa5/DenoMark/releases/latest/download/DenoMark-Encoder.tar.gz
```
...
```bash
python -m denomark.encoder.paraphrase \
...
```bash
python -m denomark.encoder.merge \
...
```bash
python -m denomark.encoder.train \
  --model_name_or_path intfloat/e5-base-v2 \
  --train_file runs/encoder_pairs/paraphrase_pairs.jsonl \
  --output_dir runs/DenoMark-Encoder \
