...
from denomark import __main__ as cli
...
def test_routes_forward_arguments(monkeypatch, path, target):
    calls = []
    monkeypatch.setattr(sys, "argv", ["denomark", *path, "--output", "some path", "--help"])
...
def test_nested_help_does_not_load_models(monkeypatch, capsys):
    monkeypatch.setattr(sys, "argv", ["denomark", "baseline"])
...
def test_invalid_command(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["denomark", "nonexistent"])
    with pytest.raises(SystemExit) as error:
        cli.main()
    assert error.value.code == 2


@pytest.mark.parametrize("package", ["denomark.core", "denomark.baselines.clean"])
